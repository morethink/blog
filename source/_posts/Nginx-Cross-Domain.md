---
title: 前端通过Nginx解决跨域问题
date: 2017-03-27
tags: 跨域
categories: 服务器
---

在前面写的一篇文章[SpringMVC 跨域](http://www.cnblogs.com/morethink/p/6525216.html)，我们探讨了什么是跨域问题以及SpringMVC怎么解决跨域问题，解决方式主要有如下三种方式:
1. JSONP
2. CORS
3. WebSocket

可是这几种方式都是基于服务器配置的，即对于自己的网站是可以通过这几种方式解决的，可是现在遇到另一个需求(前面提到过，写扇贝插件，我们不能配置扇贝的服务器)，于是，探讨了通过Nginx反向代理的方式解决跨域问题。
...
![](https://images.morethink.cn/reverse-proxy.jpg)
...
我在[Windows下实现Nginx负载均衡](http://www.cnblogs.com/morethink/p/6390072.html)提到过Nginx使用。
...
# HTML本地打开(Nginx仅处理跨域问题)
一般的情况下，我们的HTML文件时放置在Nginx服务器上面的，即通过Nginx打开，但是在前端进行调试的时候，使用服务器打开不是特别方便。而我们之所以要部署在服务器上，是想要使用浏览器自带的CORS协议来解决跨域问题，如果不想放置在Nginx等服务器中，而想通过本地打开的方式来调试HTML，可以通过自己添加`Access-Control-Allow-Origin`等http头，但是我们的AJAX请求一定要加上`http://127.0.0.1`，将nginx.conf作如下配置：
...
向我的后台是restful风格的接口，那么对于前端的就要增加配置来是进行处理。
...
# URL重写
有时候我们仅仅只想将`/api`下的url反向代理到后端，可以通过在nginx.conf中配置如下(通过url重写)：。
```
location / {
    root   html;
    index  index.html index.htm;
...
由于同源策略的限制，所读取的cookie为跨域请求接口所在域的cookie，而非当前页。如果想实现当前页cookie的写入，可在nginx反向代理中设置proxy_cookie_domain。

# 总结

Nginx解决跨域问题通过Nginx反向代理和URL重写模块将对真实服务器的请求转移到本机服务器来避免浏览器的"同源策略限制"。
